...
public class JedisUtil {
	...
	/**
	 * redis address, like "{ip}"、"{ip}:{port}"、
	 * "{redis/rediss}://xxl-sso:{password}@{ip}:{port:6379}/{db}"；Multiple ","
	 * separated
	 */
	 ...
	 * Jedis("127.0.0.1", 6379).get("cache_key"); 方式02: Redis单节点 +
	 * JedisPool单节点连接池 》》 Redis单节点压力过重，负载和容灾比较差 new JedisPool(new
	 * JedisPoolConfig(), "127.0.0.1", 6379,
	 * 10000).getResource().get("cache_key"); 方式03:
	 * Redis分片(通过client端集群,一致性哈希方式实现) + Jedis多节点连接池 》》Redis集群,负载和容灾较好,
	 * ShardedJedisPool一致性哈希分片,读写均匀，动态扩充 new ShardedJedisPool(new
	 * JedisPoolConfig(), new LinkedList<JedisShardInfo>()); 方式03: Redis集群； new
	 * JedisCluster(jedisClusterNodes); // TODO
	 */
				...
				if (INSTANCE_INIT_LOCL.tryLock(2, TimeUnit.SECONDS)) {
					...
					try {
						...
						if (shardedJedisPool == null) {
							...
							config.setMinIdle(8);
							config.setMaxWaitMillis(10000); // 获取连接时的最大等待毫秒数(如果设置为阻塞时BlockWhenExhausted),如果超时就抛异常,
															// 小于零:阻塞不确定的时间,
															// 默认-1
							...
							config.setTimeBetweenEvictionRunsMillis(30000); // 表示idle
																			// object
																			// evitor两次扫描之间要sleep的毫秒数
							config.setNumTestsPerEvictionRun(10); // 表示idle
																	// object
																	// evitor每次扫描的最多的对象数
							config.setMinEvictableIdleTimeMillis(60000); // 表示一个对象至少停留在idle状态的最短时间，然后才能被idle
																			// object
	 ...
	 *         timeout (versions lt 2.1.3), or the key does not exist.
	 */
